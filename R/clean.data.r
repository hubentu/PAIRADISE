...
clean.data <- function(my.data) {
    ...
    for (iExon in 1:nExon.raw) {
        ...
        valid <- (I1.raw[[iExon]] + S1.raw[[iExon]] != 0) & (I2.raw[[iExon]] + S2.raw[[iExon]] != 
            0)
        valid[is.na(valid)] <- FALSE
        
        if (I1.miss == M[iExon] | S1.miss == M[iExon] | I2.miss == M[iExon] | S2.miss == 
            M[iExon] | sum(valid) == 0) {
    ...
    output <- list(I1.raw, S1.raw, I2.raw, S2.raw, length_I.raw, length_S.raw, exonList.raw, 
        nExon, M)
    names(output) <- c("I1", "S1", "I2", "S2", "length_I", "length_S", "exonList", 
        "nExon", "M")
