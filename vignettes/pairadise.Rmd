...
# PAIRADISE
## PDseDataSet
A `PDseDataSet` class is defined to store the splicing count data, which contains inclusion and skipping counts for each sample. A design `dataframe` is required to descibe the paired sample information. A integer `dataframe` of inclusion and skipping lengths are also required. The `PDseDataSet` extended the `SummarizedExperiment` class. All functions from `SummarizedExperiment` are inherited.
...
A count matrix from rMATs/PAIRADISE can be imported as `PDseDataSet` directly.
```{r}
data("sample_dataset")
sample_dataset
```
...
## `pairadise` method
The `pairadise` function implemented the PAIRADISE Statistical Model for the `PDseDataSet` object. Multiple processors can be used via the `BiocParallel` pacakge. The function return a `PDseDataSet` object with statistical estimates in its `rowData`. Here is how to run the model with 2 threads.
```{r}
pdat <- pairadise(pdat, numCluster = 2)
```

## Output
A function `results` can be used to calculate p values and filter the significant results. For example, results with FDR of significant level 0.01 can be obtained as follow.
```{r}
res <- results(pdat, p.adj = "BH", sig.level = 0.01)
res
```

With `details=TRUE`, more detailed statistical estimates can be returned.
```{r}
res <- results(pdat, details = TRUE)
colnames(res)
res$latent[3,]
```
